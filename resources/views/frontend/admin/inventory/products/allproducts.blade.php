...
<div class="container card" style="padding:15px">
    ...
    <table class="table table-striped table-hover dt-responsive" cellspacing="0" width="100%" id="tableId">
        ...
        <tbody>
            ...
            <tr>
                <td>{{$key+1}}</td>
                <td> @if(isset($c->product_image))
                    <img src="{{ asset($c->product_image) }}" alt="Product" style="height: 6rem; width:6rem">
                    @else
                ...
                <td>
                <a href="generateBarcode/{{$c->unique_id}}"  class="btn btn-info" title="generate barcode">Generate Barcode</a>
                </td>
                <td>{{$c->price}}</td>
                @if($c->status==1)
